...
export async function loadTeamEhsAssignments(
  ...
  options?: { regionId?: string | null; dtEmployeeIds?: string[] }
  ...
  for (const t of teams ?? []) {
    ...
    const dtTools = tools.filter((x) => x.ehs_wear_role === "dt");
    const driverTools = tools.filter((x) => x.ehs_wear_role === "driver_rigger");
