...
async function resolveActorFromRequest(request: NextRequest): Promise<{ userId: string | null; email: string | null }> {
  const explicitId = request.headers.get("x-audit-actor-id");
  const explicitEmail = request.headers.get("x-audit-actor-email");
...
/** Fire-and-forget API access log (service role). */
