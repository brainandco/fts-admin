 ...
 * Prefers exact case-insensitive segment match; for queries of length ≥ 3, also matches folder segments
 * (no dot) that contain the query as substring.
 */
export function matchSiteFolderPaths(relativeObjectKeys: string[], queryRaw: string): Map<string, { siteSegment: string; folderPath: string }> {
