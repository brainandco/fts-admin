...
/** Junior DT, QC, QA, PP, PM, PC, Reporting Team, and support roles are not eligible for DT / Driver-Rigger / Self-DT team slots. */
  ...
  for (const id of ids) {
    ...
    if (roles.some((role) => ROLES_NOT_ALLOWED_ON_TEAM.has(role))) {
      return {
        ok: false,
        message:
          "This role cannot be on a team (Junior DT, QC, QA, PP, PM, PC, Reporting Team, or any custom / Other role).",
      };
    }
  }
  return { ok: true };
