...
export function AssignToEmployeeClient({ assets, employees }: { assets: Asset[]; employees: Employee[] }) {
  ...
  if (assets.length === 0) {
    return (
      <div className="rounded-xl border border-zinc-200 bg-white p-8 text-center">
        <p className="text-zinc-600">No available assets. Add assets first, then assign them to an employee from here.</p>
        <p className="mt-2 text-sm text-zinc-500">Assets that are already assigned do not appear here.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-4 rounded-xl border border-zinc-200 bg-white p-4">
        <div className="min-w-[200px]">
          <label className="mb-1 block text-sm font-medium text-zinc-700">Assign to employee</label>
        ...
        >
          {submitting ? "Assigning…" : `Assign ${selected.size} selected to employee`}
        </button>
        {employees.length === 0 && <p className="text-sm text-amber-600">No employees in scope. PM sees only their region; add employees first.</p>}
      </div>
