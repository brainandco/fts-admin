...
/** Short public ZIP: `/api/employee-files/pp-reports/zip-p/{folderName}/{linkId}` */
export async function GET(_req: Request, ctx: { params: Promise<{ folder: string; id: string }> }) {
  ...
  const resolved = resolveAdminPpReportsFolderZip(row.normalized_folder_path);
  if (!resolved.ok) {
